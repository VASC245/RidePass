<template>
  <div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="flex justify-center">
      <span class="bg-indigo-100 text-indigo-700 font-bold text-sm px-4 py-2 rounded-full">
    ...
    <div class="grid sm:grid-cols-3 gap-6">
      <div v-for="plan in plans" :key="plan.id"
        ...
        :class="plan.id === 'basico' ? 'border-indigo-400 shadow-indigo-100' : 'border-gray-100'">

        <!-- Popular badge -->
        <div v-if="plan.popular"
          class="absolute -top-3 left-1/2 -translate-x-1/2 bg-indigo-600 text-white text-xs font-bold px-4 py-1 rounded-full">
          Más popular
        </div>
        ...
        <div v-else
          class="w-full py-2.5 rounded-xl font-bold text-sm text-center bg-green-50 text-green-700 border border-green-200">
          ✓ Plan actual
        </div>
      </div>
    </div>
    ...
    <div v-if="pendingPlan" class="bg-blue-50 border border-blue-200 rounded-2xl p-6 text-center space-y-3">
      ...
      <div class="bg-white rounded-xl p-4 text-sm text-left space-y-1 max-w-sm mx-auto border border-blue-100">
        ...
        <p>Monto: <strong class="text-indigo-700">${{ pendingPlan.price }}/mes</strong></p>
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b">
          <tr>
            ...
            <th class="text-center px-4 py-3 font-semibold text-indigo-600">Básico</th>
            <th class="text-center px-4 py-3 font-semibold text-purple-600">Pro</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-50">
          <tr v-for="row in comparison" :key="row.label">
            ...
            <td class="px-4 py-3 text-center font-semibold text-indigo-700">{{ row.basico }}</td>
            <td class="px-4 py-3 text-center font-semibold text-purple-700">{{ row.pro }}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const comparison = [
  ...
  { label: 'WhatsApp al dueño',free: '✓',         basico: '✓',          pro: '✓'          },
  { label: 'Portal del cliente',free: '✓',        basico: '✓',          pro: '✓'          },
  { label: 'Notif. automáticas',free: '–',        basico: '✓',          pro: '✓'          },
